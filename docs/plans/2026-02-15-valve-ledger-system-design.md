...
|------|------|------|
| id | Integer | 主键 |
| username | String(50) | 用户名（使用真实姓名，唯一） |
...
|------|------|
| **布局** | 单页表单 |
| **字段** | 所属中心（必填）、设备位号（必填，自动带出设备名称）、检修时间（必填）、检修内容 |
| **设备位号** | 输入后自动关联带出设备名称 |
| **新增页面** | 打开为空表单 |
| **编辑页面** | 打开为预填好的数据，可修改 |
| **操作按钮** | 提交、返回 |
| **编辑入口** | 维护记录列表中每条记录有编辑按钮 |
...
|------|------|
| 第1步 | 选择文件 - 上传Excel |
| 第2步 | 预览 - 显示Excel基础信息列表 |
| 第3步 | 显示结果 - 成功数、失败数、错误原因 |

### 预览页面

| 功能 | 设计 |
|------|------|
| **数据显示** | 显示Excel中的基础信息列表 |
| **字段映射** | 配置文件形式，方便后期修改 |
| **冲突处理** | 显示重复位号，提供三种处理方式：全部覆盖、只导入不重复部分、取消 |

### 冲突处理方式

1. **全部覆盖** - 重复位号的数据更新现有数据
2. **只导入不重复部分** - 跳过重复数据，只导入新数据
3. **取消** - 取消导入操作
...
|--------|------|------|
| 审批模式 | 开关 | 自动审批 / 手动审批 |
| 默认初始密码 | 文本 | 新用户初始密码（默认123456） |
| 每页显示数量 | 下拉选择 | 列表每页显示条数（10/20/50） |
| 系统名称 | 文本 | 系统显示名称 |
